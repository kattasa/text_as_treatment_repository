...
#         self.model.train()
#         optimizer = AdamW(self.model.parameters(), lr=learning_rate, eps=1e-8, no_deprecation_warning=True)
...
#             data.append(torch.tensor(out[x]))
#         data = tuple(data)
...
class CausalBertWrapper:
    ...
    def ATE(self, C, W, Y=None, platt_scaling=False):
        ...
        if platt_scaling and Y is not None:
            ...
            Q0 = Q_probs[:, 0]
            Q1 = Q_probs[:, 1]
        return np.mean(Q1 - Q0), Q1, Q0
    def get_tx_preds(self, texts, confounds, outcome=None):
        self.model.eval()
        dataloader = self.build_dataloader(texts, confounds, outcomes=outcome,
            sampler='sequential')
        Ts = []
        for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
            if CUDA: 
                batch = (x.cuda() for x in batch)
            W_ids, W_len, W_mask, C, T, Y = batch
            g, _, _, _, _, _ = self.model(W_ids, W_len, W_mask, C, T, use_mlm=False)
            Ts += (g.detach().cpu().numpy() >= 0.5).tolist() # That = 1 if g >= 0.5
            # if i > 5: break
        return Ts
    
    def est_prop_score(self, texts, confounds, outcome=None):
        self.model.eval()
        dataloader = self.build_dataloader(texts, confounds, outcomes=outcome,
            sampler='sequential')
        Q0s = []
...
#     if(len(sys.argv[1]) > 1):
#         folder = sys.argv[1]
#         text = pkl.load(open(folder + '/text.pkl', 'rb'))
#         C = pkl.load(open(folder + '/C.pkl', 'rb'))
#         T = pkl.load(open(folder + '/T.pkl', 'rb'))
#         Y = pkl.load(open(folder + '/Y.pkl', 'rb'))
#     else:
...
#         T = df['T']
#         Y = df['Y']
#     cb.train(text, C, T, Y, epochs = 1)
#     print(cb.ATE(C, text, platt_scaling=True))
